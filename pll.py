...
@get('/letter/<stamp>')
def letter_listing_handler(stamp):
...
@get('/letter/<stamp>/line/<signet>')
def line_listing_handler(stamp, signet):
    """Handles name listing"""
    response.headers['Content-Type'] = 'application/json'
    response.headers['Cache-Control'] = 'no-cache'
...
@put('/letter/<stamp>/line/<signet>')
def line_update_handler(line):
